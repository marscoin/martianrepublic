...
- [x] Review and update composer dependencies for security patches -- DONE: Updated Composer 2.2.4→2.9.5. Updated all 8 vulnerable packages: laravel/framework v11.7→v11.48, livewire/livewire v3.4.12→v3.7.11, league/commonmark 2.4.2→2.8.0, symfony/http-foundation v7.0.7→v7.4.6, symfony/process v7.0.7→v7.4.5. All 12 security advisories resolved (0 remaining). Bumped to v3.0.
...
- [x] Wallet HD: Core functionality WORKING via client-side JS (my_bundle.js). Entropy generation (mouse movements), BIP39 mnemonic (12-word), BIP44 derivation (m/44'/2'/0'/0/0), PSBT transaction signing, password encryption (PBKDF2+AES), wallet creation/open flows all functional. Advanced features not implemented: multi-address derivation, change addresses, watch-only wallets, server-side recovery, xpub export. The bitcoin-php/bitcoin-ecdsa package is unused (all crypto is browser-side).
