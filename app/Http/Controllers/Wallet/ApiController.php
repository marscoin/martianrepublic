...
class ApiController extends Controller {
	...
	public function permapinpic(){

		if (Auth::check()) {
			$hash = "";
			$dataPic = Input::get('picture');
			$type = Input::get('type');
	...
	public function permapinvideo(){

		if (Auth::check()) {
			$hash = "";
			$dataPic = Input::file('file');
			$type = Input::get('type');
			$public_address = Input::get('address');
			if (Input::hasFile('file'))
			{
				...
				$file_path = "./assets/citizen/" . $public_address  . "/";
				Input::file('file')->move($file_path, "profile_video.webm" );
