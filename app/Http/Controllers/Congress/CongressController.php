...
class CongressController extends Controller
{
    ...
    public function showAll()
    {
        ...
        try {
            $resp = Http::timeout(5)->get(config('blockchain.explorer.primary_url').'/api/status?q=getInfo');
            $view->blockHeight = $resp->successful() ? ($resp->json()['info']['blocks'] ?? '---') : '---';
        } catch (\Exception $e) {
            $view->blockHeight = '---';
        }
    ...
    public function proposal($id)
    {
        if (Auth::check()) {
            ...
            $view->lifecyclePhase = GovernanceTiers::currentPhase($proposal);
            $view->tierConfig = GovernanceTiers::get($proposal->tier ?? 'signal');
    ...
    public function withdrawProposal(Request $request)
    {
        ...
        $phase = GovernanceTiers::currentPhase($proposal);
        if ($phase !== 'screening') {
    ...
    public function amendProposal(AmendProposalRequest $request)
    {
        ...
        $timestamps = GovernanceTiers::calculateTimestamps($proposal->tier);
    ...
    public function challengeTier(ChallengeTierRequest $request)
    {
        ...
        if (! GovernanceTiers::isValidReclassification($proposal->tier, $proposedTier)) {
