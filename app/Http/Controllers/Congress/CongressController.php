...
class CongressController extends Controller
{
    ...
    public function showAll()
	{
		...
		try {
			$view->blockHeight = @file_get_contents(config('blockchain.pebas.url') . '/api/mars/balance?address=MDCURC61G7A5jNRjnDq42XB1RvU51y4Ftx') ? json_decode(file_get_contents(config('blockchain.explorer.primary_url') . '/api/status?q=getInfo'))->info->blocks ?? '---' : '---';
		} catch (\Exception $e) {
			$view->blockHeight = '---';
		}
